...
public class KafkaContentConsumerTest {
    ...
    private final String interests = "China|Sri Lanka";
    ...
    public void testConstructorClose() throws Exception {
        ...
        try {
            new KafkaContentConsumer<>(props, new ByteArrayDeserializer(), new ByteArrayDeserializer());
    ...
    public void testOsDefaultSocketBufferSizes() throws Exception {
        ...
        KafkaContentConsumer<byte[], byte[]> consumer = new KafkaContentConsumer<>(
    ...
    public void testInvalidSocketSendBufferSize() throws Exception {
        ...
        new KafkaContentConsumer<>(config, new ByteArrayDeserializer(), new ByteArrayDeserializer());
    }
    ...
    public void testSubscription() {
        KafkaContentConsumer<byte[], byte[]> consumer = newConsumer();
    ...
    public void testInterceptorConstructorClose() throws Exception {
        try {
            Properties props = new Properties();
            // test with client ID assigned by KafkaContentConsumer
            props.setProperty(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:9999");
            props.setProperty(ConsumerConfig.INTERCEPTOR_CLASSES_CONFIG, MockConsumerInterceptor.class.getName());

            KafkaContentConsumer<String, String> consumer = new KafkaContentConsumer<String, String>(
    ...
    private KafkaContentConsumer<byte[], byte[]> newConsumer() {
        Properties props = new Properties();
        props.setProperty(ConsumerConfig.CLIENT_ID_CONFIG, "my.consumer");
        props.setProperty(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:9999");
        props.setProperty(ConsumerConfig.METRIC_REPORTER_CLASSES_CONFIG, MockMetricsReporter.class.getName());

        return new KafkaContentConsumer<>(props, new ByteArrayDeserializer(), new ByteArrayDeserializer());
    }
    ...
    public void verifyHeartbeatSent() throws Exception {
        ...
        final KafkaContentConsumer<String, String> consumer = newConsumer(time, client, metadata, assignor,
    ...
    private ConsumerRebalanceListener getConsumerRebalanceListener(final KafkaContentConsumer<String, String> consumer) {
    ...
    private FetchResponse fetchResponse(Map<TopicPartition, FetchInfo> fetches) {
        ...
        for (Map.Entry<TopicPartition, FetchInfo> fetchEntry : fetches.entrySet()) {
            ...
            records.close();
            tpResponses.put(partition, new PartitionData(Errors.NONE.code(), 0, records));
        }
    ...
    private KafkaContentConsumer<String, String> newConsumer(Time time,
                                                      KafkaClient client,
                                                      Metadata metadata,
        ...
        InterestCoordinator interestCoordinator = new InterestCoordinator(
                consumerClient,
                groupId,
                time,
                metadata,
        ...
        return new KafkaContentConsumer<>(
                clientId,
                consumerCoordinator,
                keyDeserializer,
                valueDeserializer,
    ...
    public void testSetInterest() {
        ...
        consumer.setInterests(interests);
        assertEquals(interests, consumer.interests());
        consumer.close();
    }
}
