...
public class PlaintextTransportLayer implements TransportLayer {
    ...
    */
    ...
    public long transferFromWithFiltering(FileChannel fileChannel, long position, long count,FilteringContent content) throws IOException {
        System.out.println("awa");
        ByteBuffer buffer = ByteBuffer.allocate((int)count);
        //ByteBuffer.allocate((int)(position+count));
        fileChannel.read(buffer, position);
        String message = new String(buffer.array());
        System.out.println("here "+message);
        buffer.flip();
        if(QueryEvaluator.filter(message,content.getExpression(),content.getTrieList())){
            System.out.println("yes thiynawa");
            socketChannel.write(buffer);
            return count;

        }else {
//            ByteBuffer buffer1 = ByteBuffer.allocate((int)count);
//            socketChannel.write(buffer1);
            return 0;
        }
//        System.out.println("ret val "+num+" sum :"+count);
        //return fileChannel.transferTo(position, count, socketChannel);
    }
}
