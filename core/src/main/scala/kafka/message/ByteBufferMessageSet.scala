...
import org.apache.kafka.common.errors.{CorruptRecordException, InvalidTimestampException}
...
class ByteBufferMessageSet(val buffer: ByteBuffer) extends MessageSet with Logging {
  ...
  // content filter method filter method
  override def filter(maxMessageSize: Int) {
