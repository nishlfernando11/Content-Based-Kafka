...
class KafkaApis(val requestChannel: RequestChannel,
  ...
  def handle(request: RequestChannel.Request) {
    try {
      ...
      ApiKeys.forId(request.requestId) match {
        ...
        case ApiKeys.SEND_INTERESTS => handleSendInterestsRequest(request)
    ...
    def sendResponseCallback(responsePartitionData: Seq[(TopicAndPartition, FetchResponsePartitionData)]) {
      val convertedPartitionData = {
        ...
        responsePartitionData.map { case (tp, data) =>
          ...
          if(ConsumerGroupInterests.isContentFilteringEnabled(request.connectionId)){
            println("yes content enables "+request.connectionId)
            records.setContentEnable(true)
            records.setFilteringContent(ConsumerGroupInterests.getGroupInterests(request.connectionId))
          }
  ...
  // method that handles send interest requests
  def handleSendInterestsRequest(request: RequestChannel.Request) {
    val sendInterestsRequest = request.body.asInstanceOf[SendInterestsRequest]
    val clientId = request.header.clientId
    val connectionId = request.connectionId
    info("Starting handling send interests for connection : "+request.connectionId)

    // the callback for sending a leave-group response
    def sendResponseCallback(errorCode: Short) {
      val response = new SendInterestsResponse(errorCode)
      trace("Sending send interests response %s for correlation id %d to client %s."
        .format(response, request.header.correlationId, request.header.clientId))
      requestChannel.sendResponse(new RequestChannel.Response(request, response))
    }
...
//      requestChannel.sendResponse(new Response(request, leaveGroupResponse))
      ...
      // let the admin manager to handle send requests
      adminManager.handleSendInterests(
        clientId,
        connectionId,
        sendInterestsRequest.interests(),
        sendResponseCallback)
    }
  }
